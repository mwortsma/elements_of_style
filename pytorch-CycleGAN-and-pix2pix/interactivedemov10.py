...
if __name__ == '__main__':
    ...
    vout = cv2.VideoWriter('output4.avi',fourcc, fps, (256,256))
    first = True
    frame_num = 0
    ...
    while(True):
        ...
        diff = (beat_times[idx] - time)
        print(time, diff)


        img = frame[:,280:1000]
        img_resize  = cv2.resize(img, (256, 256))
        img_resize = img_resize + diff*np.ones((256,256,3))#np.random.normal(0,diff*80, (256, 256, 3))
        ...
        with torch.no_grad():
            ...
            out = np.uint8(0.3*img_resize + 255*(0.7)*out)
            vout.write(out)
